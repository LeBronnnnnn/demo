...
def one_test():
    ...
    def server_run():
        server_out = os.system(config['server_exec_cmd'])
    ...
    client_exec_result = os.system(config['client_exec_cmd'])
   ...
   # if client_exec_result[1] != b'':
    ...
    # 计算服务器和客户端的md5值
    file_md5_on_client = execCmd("md5sum ../Client/output.bin")[0].split()[0]
    file_md5_on_server = execCmd("md5sum ../Server/output.bin")[0].split()[0]
