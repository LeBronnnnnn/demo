...
class UDPServer
{
	...
	void createSocket();
	void bindAddress(const char *ip,int portNumber);
 ...
 */
void UDPServer::bindAddress(const char *ip,int portNumber)
{
	int leng=sizeof(serverAddress);
	bzero(&serverAddress,leng);
	serverAddress.sin_family = AF_INET;
	serverAddress.sin_port = htons(portNumber); 
	inet_aton(ip, &serverAddress.sin_addr);//设置ip    	
...
int main(int noOfArguments,char *argumentList[])
{
	...
	server.bindAddress(argumentList[1],portNo);
	server.setClientSockLength();	
	while(1)
	{
		server.receiveRequest();
		char* fileContent=server.getRequestedContent();
		server.createSegments(fileContent,atoi(argumentList[3]));	//ip+port+window	
	}
	return 0;	
}
